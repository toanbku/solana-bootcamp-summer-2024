...
declare_id!("16RGbKergxtWkpfHe21ZqEXk9x3G8tuTb2X8dT42Ge8");
...
pub fn toggle_todo(ctx: Context<ToggleTodo>) -> Result<()> {
        require!(ctx.accounts.profile.authority == ctx.accounts.creator.key(), AppError::InvalidAuthority);
        require!(ctx.accounts.todo.profile == ctx.accounts.profile.key(), AppError::InvalidAuthority);
        let todo = &mut ctx.accounts.todo;
